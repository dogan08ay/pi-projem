...
async function verifyAppCheckSoft(req, action) {
  ...
  const debugReason = req.headers['x-appcheck-debug']; // YENİ: istemcinin bildirdiği teşhis sebebi
  if (!token) {
    ...
    if (debugReason && _appCheckSoftLogCount <= 20) {
      _appCheckSoftLogCount++;
      console.warn(`[AppCheck-Debug] İstemci token gönderemedi (action:${action}) — istemci tarafı sebep: ${debugReason}`);
    }
